...
describe Diffaroo::Matcher do
  ...
  describe "#matches" do
    it "returns the array of Match objects" do
      ...
      matcher.matches.should be_instance_of(Array)
      matcher.matches.first.should be_instance_of(Diffaroo::Match)
    end
  end
  ...
  describe "#pairs" do
    it "returns the collected node pairs from the matches" do
      ...
      doc2    = xml { root2 { a1 ; a2 } }
      matcher = Diffaroo::Matcher.new(doc1, doc2)
      matcher.pairs.should == [ [doc1.at_css("a1"), doc2.at_css("a1")],
  ...
  describe "basic node matching" do
    context "simple matches" do
      ...
      it "does not match different nodes" do
        matcher = Diffaroo::Matcher.new(@doc1, @doc2)
        matcher.pairs.flatten.should_not include(@doc1.at_css("b1"))
        matcher.pairs.flatten.should_not include(@doc2.at_css("b2"))
      end
    end
    ...
    context "nested matches" do
      ...
      it "matches the largest identical subtree" do
        matcher = Diffaroo::Matcher.new(@doc1, @doc2)
        matcher.pairs.should include([@doc1.at_css("root"), @doc2.at_css("root")])
      end

      it "does not match small nodes within a larger matching subtree" do
        matcher = Diffaroo::Matcher.new(@doc1, @doc2)
        matcher.pairs.flatten.should_not include(@doc1.at_css("a1"))
        matcher.pairs.flatten.should_not include(@doc2.at_css("a1"))
      end
    end
  end
  ...
  describe "forced parent matching" do
    ...
    it "large subtree matches force more parent matches than smaller subtree matches" do
      ...
      small_pairs = small_matcher.pairs
      ...
      large_pairs = large_matcher.pairs
  ...
  describe "choosing the best among multiple possible matches" do
    ...
    context "one match's parent is same-named" do
      it "matches the node with the same-named parent" do
        ...
        matcher = Diffaroo::Matcher.new(doc1, doc2)
        matcher.pairs.should include([doc1.at_css("a2"), doc2.at_css("a2")])
      end
    end
    ...
    context "multiple matches' parents are same-named" do
      it "matches the node with the same-named grandparent" do
        ...
        matcher = Diffaroo::Matcher.new(doc1, doc2)
        matcher.pairs.should include([doc1.at_css("wrap2"), doc2.at_css("wrap2")])
      end
    end
  end
end
